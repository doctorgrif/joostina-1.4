...
class BossImageGalleryPlugin
{
    ...
    function getFormDisplay($directory, $content, $field, $field_values, $nameform = 'adminForm', $mode = "write")
    {
        ...
        if (!is_dir(_JLPATH_ROOT . $path . 'origin')) {
            self::makePath(_JLPATH_ROOT, $path . 'origin');
        }

        if (!is_dir(_JLPATH_ROOT . $path . 'full')) {
            self::makePath(_JLPATH_ROOT, $path . 'full');
        }

        if (!is_dir(_JLPATH_ROOT . $path . 'thumb')) {
            self::makePath(_JLPATH_ROOT, $path . 'thumb');
        }

        $mainframe = mosMainFrame::getInstance();
        $mainframe->addJS(_JLPATH_SITE . '/administrator/components/com_boss/js/upload.js');
        $mainframe->addJS(_JLPATH_SITE . '/components/com_boss/plugins/fields/BossImageGalleryPlugin/js/script.js');
    ...
    function onFormSave($directory, $contentid, $field, $isUpdateMode)
    {
        ...
        if (!empty($conf['tag']->fieldvalue)) {
            ...
            $thumb->Copyrightfonttype = _JLPATH_ROOT . '/components/com_boss/font/verdana.ttf';
        ...
        foreach ($boss_img_gallery as $boss_img) {
            ...
            // image1 upload
            $origin = _JLPATH_ROOT . "/images/boss/" . $directory . "/contents/gallery/origin/" . $filename;

            //если есть оригинальный файл
            if (is_file($origin)) {
                //если нет эскиза
                if (!is_file(_JLPATH_ROOT . "/images/boss/" . $directory . "/contents/gallery/full/" . $filename)) {
                    $thumb->Thumbsize = $conf['max_size']->fieldvalue;
                    $thumb->Thumblocation = _JLPATH_ROOT . "/images/boss/$directory/contents/gallery/full/";
                    $thumb->Createthumb($origin, 'file');
                }

                //если нет миниатюры
                if (!is_file(_JLPATH_ROOT . "/images/boss/" . $directory . "/contents/gallery/thumb/" . $filename)) {
                    $thumb->Thumbsize = $conf['max_size_t']->fieldvalue;
                    $thumb->Thumblocation = _JLPATH_ROOT . "/images/boss/" . $directory . "/contents/gallery/thumb/";
                    $thumb->Createthumb($origin, 'file');
                }
            }
        }
            ...
            foreach ($file_fields as $file_field) {
                ...
                if (is_array($files) && count($files) > 0) {
                    foreach ($files as $file) {
                        @unlink(_JLPATH_ROOT . "/images/boss/$directory/contents/gallery/origin/" . $file->file);
                        @unlink(_JLPATH_ROOT . "/images/boss/$directory/contents/gallery/full/" . $file->file);
                        @unlink(_JLPATH_ROOT . "/images/boss/$directory/contents/gallery/thumb/" . $file->file);
                    }
                }
            }
        }
    ...
    function addInHead($field, $field_values, $directory)
    {
        ...
        $path = _JLPATH_SITE . '/components/com_boss/plugins/fields/BossImageGalleryPlugin/engines/' . $engine;
        $params = array();

        //загружаем файл инциализации скрипта галереи
        self::getEngine($engine);
        $params = galleryAddInHead($path);
        //$params['css']['galleryImg0'] = _JLPATH_SITE . '/components/com_boss/plugins/fields/BossImageGalleryPlugin/css/style.admin.css';
    ...
    private function displayFileLink($directory, $filename)
    {
        $return = '';
        if ($filename) {
            $return .= "<img class='thumb' src=\"" . _JLPATH_SITE . "/images/boss/" . $directory . "/contents/gallery/thumb/" . $filename . "\" align=\"middle\" border=\"0\" />&nbsp;";
        }
