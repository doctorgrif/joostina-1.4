...
(function ($) {
    ...
    $.mbGallery = {
        ...
        buildMbGallery:function (options) {
            ...
            if (gallery.options.onOpen) {
                gallery.options.onOpen();
            }
            ...
            var galleryImg = $("<div/>")
                .addClass("galleryImg")
                .hover(function () {
                    if (galleryDesc.html()) galleryDesc.slideDown();
                }, function () {
                    galleryDesc.slideUp();
                })
                .dblclick(function () {
                    if (gallery.sliding) $(gallery).mb_stopSlide(); else $(gallery).mb_startSlide();
                });
        ...
        getPhotos:function () {
            ...
            $(gallery).find(gallery.options.thumbnailSelector).each(
                function (i) {
        ...
        preload:function () {
            ...
            var rndVar = "?" + new Date().getTime();
            var showExif = gallery.options.exifData;

            $("<img/>").load(
                function () {
                    if (!gallery.sliding) $("#" + gallery.galleryID).find(".loader").fadeOut(500, function () {
                        $("#" + gallery.galleryID).find(".loader").removeClass("loading");
                    });
        ...
        changePhoto:function (rndVar) {
            var gallery = $(this).get(0);
            $("#" + gallery.galleryID).find(".loader").fadeOut(500, function () {
                $("#" + gallery.galleryID).find(".loader").removeClass("loading");
            });
            ...
            newImg.fadeIn("slow", function () {
                galleryNav.fadeIn(500);
                galleryTitle.fadeIn()
            });
            newImg.next("img").fadeOut("slow", function () {
                $(this).remove();
            });
            photoTitle.fadeOut("slow", function () {
                photoTitle.html(gallery.images[gallery.idx].title);
                photoTitle.fadeIn();
            });
    ...
    jQuery.fn.extend(
        {
            getDim:function (gallery, w, h) {
                ...
                if (gallery.options.galleryMaxHeight > 0 && $(this).outerHeight() > gallery.options.galleryMaxHeight) {
                    nh = gallery.options.galleryMaxHeight;
                }
                if (gallery.options.galleryMaxWidth > 0 && $(this).outerWidth() > gallery.options.galleryMaxWidth) {
                    nw = gallery.options.galleryMaxWidth;
                }
