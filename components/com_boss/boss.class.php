...
class jDirectoryCategory extends mosDBTable
{
    ...
    public static function saveCategory($directory, $conf)
    {
        ...
        if ($_POST['cb_image'] == "delete") {
            $pict = _JLPATH_ROOT . "/images/boss/$directory/categories/" . $row->id . "cat.jpg";
            if (file_exists($pict)) {
                unlink($pict);
            }
            $pict = _JLPATH_ROOT . "/images/boss/$directory/categories/" . $row->id . "cat_t.jpg";
            if (file_exists($pict)) {
                unlink($pict);
            }
        }
        ...
        if (isset($_FILES['cat_image']) and !$_FILES['cat_image']['error']) {
            $path = _JLPATH_ROOT . "/images/boss/$directory/categories/";
            createImageAndThumb(
...
class jDirectoryContent extends mosDBTable
{
    ...
    public static function newContent($directory, $conf)
    {
        ...
        //get fields
        $fields = $database->setQuery("SELECT * FROM #__boss_" . $directory . "_fields WHERE published = 1 AND `profile` = 0 AND (FIND_IN_SET($type_content, `catsid`) > 0 OR `catsid` = ',-1,') ORDER by ordering")
            ->loadObjectList();
        if ($database->getErrorNum()) {
            echo $database->stderr();
            return false;
    ...
    public static function displayContent($directory, $conf)
    {
        ...
        //get fields
        $database->setQuery(
            "SELECT * FROM #__boss_" . $directory . "_fields WHERE `published` = 1 AND `profile` = 0 AND (FIND_IN_SET($row->type_content, `catsid`) > 0 OR `catsid` = ',-1,') ORDER BY `ordering`, `fieldid`"
        );
...
class jDirectoryField extends mosDBTable
{
    ...
    public static function editField($directory)
    {
        ...
        $path = _JLPATH_ROOT . "/images/boss/$directory/fields";
        $handle = opendir($path);
...
class boss_helpers
{
    ...
    public static function addDirectoryScript($directory)
    {
        ...
        (is_file(_JLPATH_ROOT . $script)) ? $mainframe->addJS(_JLPATH_SITE . $script) : null;
    }
    ...
    public static function copy_folder_rf($pathFrom, $pathTo)
    {
        ...
        mosMakePath(_JLPATH_ROOT, str_replace(_JLPATH_ROOT, '', $pathTo));
    ...
    {
        $defaultLangPath = _JLPATH_ROOT . DS . 'components' . DS . 'com_boss' . DS . 'lang' . DS;
        $directoryLangPath = _JLPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . 'lang/';
        if (!$directory) {
            if (is_file($defaultLangPath . JCore::getCfg('lang') . '.php')) {
                require_once($defaultLangPath . JCore::getCfg('lang') . '.php');
            } else {
            ...
            if (is_file($directoryLangPath . JCore::getCfg('lang') . '.php')) {
                require_once($directoryLangPath . JCore::getCfg('lang') . '.php');
            } else {
    ...
    {
        $langPath = _JLPATH_ROOT . '/components/com_boss/plugins/' . $typePlugin . '/' . $plugin . '/lang/';

        if (is_file($langPath . JCore::getCfg('lang') . '.php')) {
            require_once($langPath . JCore::getCfg('lang') . '.php');
    ...
    {
        $file = _JLPATH_ROOT . "/images/boss/$directory/fields/" . basename($_FILES['uploadfile']['name']);
    ...
    public static function upload_file($directory, $folder)
    {
        ...
        //целевой файл
        $file = _JLPATH_ROOT . "/images/boss/$directory/$folder/" . $filename;
        //если целевой файл есть, то даем ему уникальное имя
        if (is_file($file)) {
            $i = 0;
            $fname = $filename;
            while (file_exists(_JLPATH_ROOT . "/images/boss/$directory/$folder/" . $fname)) {
                $i++;
                $fname = $i . "_" . $filename;
            }
            $file = _JLPATH_ROOT . "/images/boss/$directory/$folder/" . $fname;
            $filename = $fname;
        }
        if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file)) {
            echo $filename;
    ...
    public static function delete_pack($directory)
    {
        ...
        $file = _JLPATH_ROOT . "/images/boss/$directory/$pack.zip";
        if (is_file($file)) {
            unlink($file);
        }
        if (is_file($file)) {
    ...
    public static function delete_file($directory)
    {
        ...
        $file = _JLPATH_ROOT . "/images/boss/$directory/$folder/$file";
        if (is_file($file)) {
            unlink($file);
        }
        if (is_file($file)) {
    ...
    public static function get_pathlist($cats, $catid, $catname, $list, $order, $directory)
    {
        ...
        if (isset($cats)) {
            ...
            $list[$i]->link = JSef::getUrlToSef('index.php?option=com_boss&amp;task=show_category&amp;catid=' . $catid . '&amp;order=' . $order . '&amp;directory=' . $directory);
            $i++;
            ...
            if ($catid != -1) {
                ...
                while ($orderlist[$current]->parent != 0) {
                    ...
                    $list[$i]->link = JSef::getUrlToSef('index.php?option=com_boss&amp;task=show_category&amp;catid=' . $orderlist[$current]->id . '&amp;order=' . $order . '&amp;directory=' . $directory);
                    $i++;
                }
            }
        }
    ...
    public static function show_list(
        ...
        $database->setQuery(
            "SELECT g.name as gname,f.* FROM #__boss_" . $directory . "_groupfields as fg " . "LEFT JOIN #__boss_" . $directory . "_groups AS g ON fg.groupid = g.id " . "LEFT JOIN #__boss_" . $directory
                . "_fields AS f ON fg.fieldid = f.fieldid " . "WHERE g.published = 1 $groupids AND f.published = 1 AND fg.type_tmpl = 'category' " . "ORDER BY fg.ordering ASC "
        );
    ...
    public static function removeDirectory($id)
    {
        ...
        $database->setQuery(
            "DROP TABLE IF EXISTS `#__boss_" . $id . "_categories`, " . "`#__boss_" . $id . "_contents`, " . "`#__boss_" . $id . "_content_category_href`, " . "`#__boss_" . $id . "_content_types`, " . "`#__boss_" . $id
                . "_field_values`, " . "`#__boss_" . $id . "_fields`, " . "`#__boss_" . $id . "_groupfields`, " . "`#__boss_" . $id . "_groups`, " . "`#__boss_" . $id . "_profile`, " . "`#__boss_" . $id . "_rating`, "
                . "`#__boss_" . $id . "_reviews`; "
        )->query();

        $database->setQuery("DELETE FROM `#__boss_config` WHERE `id` = $id")->query();

        boss_helpers::rmdir_rf(_JLPATH_ROOT . "/images/boss/$id");
    }
}
...
class BossTemplates
{
    ...
    {
        $templates = array();
        $path = _JLPATH_ROOT . '/templates/com_boss';
        if (!is_dir($path)) {
    ...
    public static function deleteTemplate($directory)
    {
        ...
        foreach ($tid as $template) {
            if ($template != "") {
                boss_helpers::rmdir_rf(_JLPATH_ROOT . "/templates/com_boss/" . $template);
            }
...
class BossTemplateFields
{
    ...
    public static function editTmplSource($directory, $template, $source_file, $conf)
    {
        $files = array();
        $source = '';
        $path = _JLPATH_ROOT . "/templates/com_boss/$template";
        $files[] = "css/boss.css";
        $handle = opendir($path);
        while ($file = readdir($handle)) {
    ...
    {
        $path = _JLPATH_ROOT . "/templates/com_boss/$template/$source_file";
    ...
    public static function getTemplateFields($directory, $template, $type_tmpl = '')
    {
        ...
        if (count($fields) == 0) {
            $positions = array();
            require(_JLPATH_ROOT . "/templates/com_boss/$template/_service.php");
            $q = "INSERT INTO #__boss_" . $directory . "_groups " . "(`name`, `desc`, `template`, `type_tmpl`, `published`, `catsid`) " . "VALUES ";
            $q1 = array();
...
class BossPlugins
{
    ...
    {
        $bossPlugins = array();
        $path = _JLPATH_ROOT . '/components/com_boss/plugins/' . $folder . '/';
        $handle = opendir($path);
        while ($file = readdir($handle)) {
            if (($file != ".") && ($file != "..")) {
    ...
    {
        $path = _JLPATH_ROOT . '/components/com_boss/plugins/' . $folder . '/' . $name . '/';
    ...
    public static function run_plugins_func($directory)
    {
        ...
        $path = _JLPATH_ROOT . '/components/com_boss/plugins/fields/' . $class . '/plugin.php';
        if (is_file($path)) {
            require_once($path);
        } else {
            return false;
    ...
    public static function deletePlugin($directory)
    {
        ...
        foreach ($tid as $pluginname) {
            $bossPlugins = array();
            $path = _JLPATH_ROOT . "/images/boss/$directory/plugins/$pluginname";
    ...
    public static function installPlugin($directory)
    {
        ...
        foreach ($directories as $dir) {
            if ($ext == 'zip') {
                // Extract functions
                $ret = $zipfile->extract(PCLZIP_OPT_PATH, _JLPATH_ROOT . "/images/boss/$dir/plugins/$folder");
                if ($ret == 0) {
                    $zipfile->setError(1, 'Unrecoverable error "' . $zipfile->errorName(true) . '"');
                    return false;
                }
            } else {
                if (!$archive->extractModify(_JLPATH_ROOT . "/images/boss/$dir/plugins/$folder", '')) {
                    $archive->setError(1, 'Extract Error');
                    return false;
                }
            }
            require_once(_JLPATH_ROOT . "/images/boss/$dir/plugins/$folder/$name/plugin.php");
    ...
    public static function listPlugins($directory, $conf)
    {
        ...
        $path = _JLPATH_ROOT . "/images/boss/$directory/plugins";
        if (!is_dir($path)) {
            mkdir($path);
        }
...
class bossFieldImages
{
    ...
    public static function deleteFieldImage($directory)
    {
        ...
        foreach ($tid as $filename) {
            if ($filename != "") {
                @unlink(_JLPATH_ROOT . "/images/boss/$directory/fields/" . $filename);
            }
        }
    ...
    public static function uploadFieldImage($directory)
    {
        ...
        while (file_exists(_JLPATH_ROOT . "/images/boss/$directory/fields/" . $filename)) {
            $filename = "copy_" . $filename;
        }
        is_file($userfile['tmp_name']) ? move_uploaded_file($userfile['tmp_name'], _JLPATH_ROOT . "/images/boss/$directory/fields/" . $filename) : null;
...
class bossExportImport
{
    ...
    public static function showImpExpForm($directory, $conf)
    {
        ...
        $directories = BossDirectory::getDirectories();
        $packs = array();
        if (is_dir(_JLPATH_ROOT . '/images/boss/' . $directory)) {
            if ($handle = opendir(_JLPATH_ROOT . '/images/boss/' . $directory)) {
                while (false !== ($file = readdir($handle))) {
    ...
    public static function exportDirectory($directory)
    {
        ...
        $patch = _JLPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . $pack_name;
        $patchToTemplates = _JLPATH_ROOT . DS . 'templates' . DS . 'com_boss';
        $patchToPlugins = _JLPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . 'plugins';
        $patchToContentFolders = _JLPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory;
        ...
        //архивируем файлы
        zipFolder($patch, _JLPATH_ROOT . DS . 'images' . DS . 'boss' . DS . $directory . DS . $pack_name . '.zip');
    ...
    public static function importDirectory()
    {
        ...
        mosMakePath(_JLPATH_ROOT . "/images", 'boss/tmp');
        //разархивируем во временную папку
        $ret = $zipfile->extract(PCLZIP_OPT_PATH, _JLPATH_ROOT . "/images/boss/tmp");
        if ($ret == 0) {
        ...
        $pathFrom = _JLPATH_ROOT . "/images/boss/tmp";
        //путь куда копировать
        $pathTo = _JLPATH_ROOT . "/images/boss/$directory";
        ...
        is_dir($pathFrom . '/templates') ? boss_helpers::copy_folder_rf($pathFrom . '/templates', _JLPATH_ROOT . "/templates/com_boss") : null;
        ...
        $pathToReserve = _JLPATH_ROOT . '/components/com_boss';
