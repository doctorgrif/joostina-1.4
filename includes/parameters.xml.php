...
class mosParameters
{
        ...
        if (in_array('_form_' . $type, $this->_methods)) {
            ...
            if (mosGetParam($_REQUEST, 'option', '') == 'com_modules') {
                ...
                if ($id > 0) {
                    ...
                    if (is_file(_JLPATH_ROOT . '/modules/' . $module . '/elements.php')) {
                        require_once(_JLPATH_ROOT . '/modules/' . $module . '/elements.php');
    ...
    private function _form_directory($name, $value, $node, $control_name)
    {
        ...
        array_unshift($options, mosHTML::makeOption('0', _SELECT_DIRECTORY, 'id', 'name'));
    ...
    {
        $path = _JLPATH_ROOT . DS . $node->getAttribute('directory');
        ...
        if (intval($node->getAttribute('hide_none'))) {
            array_unshift($options, mosHTML::makeOption('-1', $const1));
        }
        if (intval($node->getAttribute('hide_default'))) {
            array_unshift($options, mosHTML::makeOption('', $const2));
        }
        return mosHTML::selectList($options, '' . $control_name . '[' . $name . ']', 'class="inputbox"', 'value', 'text', $value);
    }
    ...
    function _form_tabs($name, $value, $param, $control_name, $label)
    {
        ...
        switch ($value) {
            case 'startPane':
                ...
                $mainframe->addCSS(_JLPATH_SITE . '/includes/js/tabs/tabpane.css');
                $mainframe->addJS(_JLPATH_SITE . '/includes/js/tabs/tabpane.js');
