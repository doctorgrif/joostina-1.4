...
class JFile{
	...
	function copy($src, $dest, $path = null){
		...
		if($FTPOptions['enabled'] == 1){
			...
			$dest = JPath::clean(str_replace(_JLPATH_ROOT, $FTPOptions['root'], $dest), '/');
	...
	function delete($file){
		...
		foreach($files as $file){
			...
			if(@unlink($file)){
				...
				$file = JPath::clean(str_replace(_JLPATH_ROOT, $FTPOptions['root'], $file), '/');
				if(!$ftp->delete($file)){
					// FTP connector throws an error
					return false;
				}
	...
	function move($src, $dest, $path = ''){
		...
		if($FTPOptions['enabled'] == 1){
			...
			$src = JPath::clean(str_replace(_JLPATH_ROOT, $FTPOptions['root'], $src), '/');
			$dest = JPath::clean(str_replace(_JLPATH_ROOT, $FTPOptions['root'], $dest), '/');
