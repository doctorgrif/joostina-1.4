...
class SpawFm
{
    public $thumbnails_enabled = false;
    public $current_dir = false;
    public $current_dir_data = false;
    public $current_type = false;
	...
	public function normalizeDir($dir, $stripslashes = false, $noleadslash = false)
	{
		if (!strlen($dir)) {
			return false;
		}
	...
	{
		$icons = $this->getFileIconsDefinition($filename);
		return $icons['icon'];
	}

    public function getFileIconBig($filename)
	{
		$icons = $this->getFileIconsDefinition($filename);
		return $icons['icon_big'];
	}

    public function getFileIconsDefinition($filename)
	{
		global $config;
    ...
    public function getFileThumbnail($filename)
	{
		$icons = $this->getFileIconsDefinition($filename);
    ...
    function isImage($file)
	{
		...
		if (in_array($ext, $this->imagescapeHtmle_ext)) {
			return true;
		}
		return false;
	}

    public function isThumbailsModePossible($dir_path)
	{
	...
	function uploadFile($uplfile)
	{
		...
		if (!$this->getCurrentDirSetting('allow_upload')) {
			...
			if (is_uploaded_file($uplfile['tmp_name'])) {
				// check filetype
				$ext = $this->getFileExtension($uplfile['name']);
				$allowed_ext = $this->getAllowedExtensions();
					...
					if (!$this->getCurrentDirSetting('max_upload_filesize') or
						...
						*/
						...
						if (!$ok) {
							...
							$filename = convert_cyr_string($uplfile['name'], 'w', 'k');
							for ($i = 0; $i < strlen($filename); $i++) {
								$filename{$i} = chr(ord($filename{$i}) & 127);
							}
							//---------уберем сомнительные символы
							$filename = strtolower(preg_replace('/[^\w\._]/', '', $filename));
							$uplfile_name = $filename;
							$i = 1;
							// pick unused file name
							while (file_exists($this->getCurrentFsDir() . $uplfile_name)) {
								$uplfile_name = preg_replace('#(.*)(\.[a-zA-Z]+)$#', '\1_' . $i . '\2', $filename);
								$i++;
							}
