...
$upload_allow = (in_array('all', $upload_allow)) ? array('all') : $upload_allow;
//права на папки  и файлы
$dir_mode = (!empty($dir_mode)) ? octdec($dir_mode) : 0755;
$file_mode = (!empty($file_mode)) ? octdec($file_mode) : 0644;

//подготавливаем рут алиас
$root_alias = (!empty($root_alias)) ? $root_alias : 'Home';
...
//файлы с точкой показывать?
$dot_files = (isset($dot_files)) ? $dot_files : 0;

//считать размер папок?
$dir_size = (@$dir_size == 1) ? true : false;

//выбор библиотеки обработки изображений
$img_lib = (!empty($img_lib)) ? $img_lib : 'auto';
...
//кол-во тумбинашек за один проход
$tmb_at_once = (@$tmb_at_once > 0) ? $tmb_at_once : 0;

//размер тумбинашек
$tmb_size = (@$tmb_size > 0) ? $tmb_size : 30;

//показывать урл файла?
...
$opts = array(
	...
	'disabled'     => $disabled_command[$my->gid], // list of not allowed commands
	'dotFiles'     => $dot_files, // display dot files
	'dirSize'      => $dir_size, // count total directories sizes
	'fileMode'     => $file_mode, // new files mode
	'dirMode'      => $dir_mode, // new folders mode
	...
	'imgLib'       => $img_lib, // image manipulation library (imagick, mogrify, gd)
	'tmbDir'       => $tmb_dir, // directory name for image thumbnails. Set to "" to avoid thumbnails generation
	'tmbCleanProb' => $tmb_clean_prob, // how frequiently clean thumbnails dir (0 - never, 100 - every init request)
	'tmbAtOnce'    => $tmb_at_once, // number of thumbnails to generate per request
	'tmbSize'      => $tmb_size, // images thumbnails size (px)
	'fileURL'      => $file_url, // display file URL in "get info"
...
interface elFinderILogger{
	function log($cmd, $ok, $context, $err = '', $errorData = array());
}
