...
if (JCore::getCfg('one_template') != '...') {
    echo joost_info(_INOGLOBAL_CONFIG_ONE_TEMPLATE_USING . ' <b>' . JCore::getCfg('one_template') . '</b>');
}
...
function viewTemplates($option, $client)
{
    ...
    if ($client == 'admin') {
        ...
        $templateBaseDir = mosPathName(_JLPATH_ROOT . '/templates');
    }
...
function editTemplateSource($p_tname, $option, $client)
{
    ...
    if ($client == 'admin') {
        ...
        $file = _JLPATH_ROOT . '/templates/' . $p_tname . '/index.php';
    }
...
function editTemplateCSS($p_tname, $option, $client)
{
    ...
    if ($client == 'admin') {
        ...
        $file = _JLPATH_ROOT . '/templates/' . $p_tname . '/css/template_css.css';
    }
...
function assignTemplate($p_tname, $option, $client)
{
    ...
    $menulist = mosAdminMenus::MenuLinks($lookup, 1, 0);
