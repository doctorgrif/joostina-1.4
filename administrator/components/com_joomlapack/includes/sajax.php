...
if(!isset($SAJAX_INCLUDED)){
	...
	*/
	...
	function sajax_get_js_repr($value){
		...
		if($type == "boolean"){
			...
			foreach($value as $k => $v){
				$esc_key = sajax_esc($k);
				if(is_numeric($k)) $s .= "$k: " . sajax_get_js_repr($v) . ", ";
				else $s .= "\"$esc_key\": " . sajax_get_js_repr($v) . ", ";
			}
			if(count($value)) $s = substr($s, 0, -2);
			return $s . " }";
		} else{
	...
	function sajax_handle_client_request(){
		...
		if(!in_array($func_name, $sajax_export_list)) echo "-:$func_name not callable";
		else{
			echo "+:";
			ob_flush();
