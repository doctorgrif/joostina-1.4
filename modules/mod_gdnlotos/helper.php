...
class mod_gdnlotos_Helper
{
    ...
    private function getParams($params, $moduleid)
    {
        ...
        if ($this->template_dir and is_dir(_JLPATH_ROOT . DS . 'templates' . DS . JTEMPLATE . DS . 'html' . DS . 'modules' . DS . 'mod_gdnlotos' . DS . $this->template)) {
            $this->tpl_dir = _JLPATH_ROOT . DS . 'templates' . DS . JTEMPLATE . DS . 'html' . DS . 'modules' . DS . 'mod_gdnlotos' . DS . $this->template;
            $this->tpl_dir_style = _JLPATH_SITE . '/templates/' . JTEMPLATE . '/html/modules/mod_gdnlotos/' . $this->template . '/style.css';
        } elseif (is_dir(_JLPATH_ROOT . DS . 'modules' . DS . 'mod_gdnlotos' . DS . 'templates' . DS . $this->template)) {
            $this->tpl_dir = _JLPATH_ROOT . DS . 'modules' . DS . 'mod_gdnlotos' . DS . 'templates' . DS . $this->template;
            $this->tpl_dir_style = _JLPATH_SITE . '/modules/mod_gdnlotos/templates/' . $this->template . '/style.css';
        } else {
            $this->tpl_dir = _JLPATH_ROOT . DS . 'modules' . DS . 'mod_gdnlotos' . DS . 'templates' . DS . 'default';
            $this->tpl_dir_style = _JLPATH_SITE . '/modules/mod_gdnlotos/templates/default/style.css';
        }
        if ($this->css_dir) {
    ...
    private function getAssembly()
    {
        ...
        if ($this->modul_link) {
            if ($this->modul_link == 1) {
                ...
                $link = JSef::getUrlToSef("index.php?option=com_boss&amp;task=show_category&amp;directory=" . $this->directory . "&amp;catid=" . $this->modul_link_cat);
            }
    ...
    private function getItem($row, $file)
    {
        ...
        if ($this->image) {
            if ($row['type'] == 'BossImagePlugin') {
                ...
                if (is_readable(_JLPATH_ROOT . $path . 'jpg')) {
                    $text_with_image = '<img src="' . _JLPATH_SITE . $path . 'jpg">';
                } elseif (is_readable(_JLPATH_ROOT . $path . 'gif')) {
                    $text_with_image = '<img src="' . _JLPATH_SITE . $path . 'gif">';
                } elseif (is_readable(_JLPATH_ROOT . $path . 'png')) {
                    $text_with_image = '<img src="' . _JLPATH_SITE . $path . 'png">';
                } else {
                    $text_with_image = '';
                }
            } else {
            ...
            if ((isset($parse_url['host']) and $parse_url['host'] == $parse_site['host']) or !isset($parse_url['host'])) {
                if (is_readable(_JLPATH_ROOT . $parse_url['path'])) {
                    $tmp_img = _JLPATH_SITE . $parse_url['path'];
            ...
            // формируем миниэскиз
            $src = _JLPATH_SITE . '/modules/mod_gdnlotos/imgsketch.php?' . 'src=' . $tmp_img . '&w=' . $img_thumb_width . '&h=' . $img_thumb_height . '&q=' . $image_quality;
